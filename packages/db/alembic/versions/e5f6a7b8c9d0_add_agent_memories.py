...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_agent_memories_agent_key", "agent_memories", ["agent_id", "key"]
    )
    op.create_index(
        "ix_agent_memories_agent_type", "agent_memories", ["agent_id", "memory_type"]
    )
    op.create_index(
        "ix_agent_memories_tenant_agent", "agent_memories", ["tenant_id", "agent_id"]
    )
    ...
    op.add_column(
        "agents",
        sa.Column(
            "traffic_weight", sa.Numeric(5, 2), nullable=True, server_default="100"
        ),
    )
    op.add_column(
        "agents",
    ...
    op.add_column(
        "executions", sa.Column("confidence_score", sa.Numeric(3, 2), nullable=True)
    )
    ...
    op.add_column(
        "tenants", sa.Column("daily_cost_limit", sa.Numeric(10, 2), nullable=True)
    )
    op.add_column(
        "tenants", sa.Column("monthly_cost_limit", sa.Numeric(10, 2), nullable=True)
    )

    # Add per-agent cost limit
    op.add_column(
        "agents",
        sa.Column("per_execution_cost_limit", sa.Numeric(10, 4), nullable=True),
    )
    op.add_column(
        "agents", sa.Column("daily_cost_limit", sa.Numeric(10, 2), nullable=True)
    )
