...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "redaction_mask", sa.String(40), server_default="█████", nullable=False
        ),
        sa.Column(
    ...
    op.create_index(
        "ix_moderation_events_execution", "moderation_events", ["execution_id"]
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_moderation_policies_tenant_active", table_name="moderation_policies"
    )
