...
def upgrade() -> None:
    ...
    op.add_column("users", sa.Column("stripe_connect_id", sa.String(255), nullable=True))
    op.add_column("users", sa.Column("stripe_connect_onboarded", sa.Boolean(), server_default="false", nullable=False))

    op.add_column("agents", sa.Column("rejection_reason", sa.Text(), nullable=True))

    op.add_column("subscriptions", sa.Column("stripe_payment_intent_id", sa.String(255), nullable=True))

    op.create_table(
        "payouts",
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("creator_id", UUID(as_uuid=True), sa.ForeignKey("users.id"), nullable=False, index=True),
