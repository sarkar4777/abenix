...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "node_kind", sa.String(64), nullable=False
        ),  # tool|agent|http|switch|map|reduce
        sa.Column(
            "node_target", sa.String(255), nullable=True
        ),  # tool name or agent id
        # What went wrong
        ...
        # Structured payloads
        sa.Column(
            "expected_shape", postgresql.JSONB(), nullable=True
        ),  # JSONSchema-like, derived from past successes
        sa.Column(
            "observed_shape", postgresql.JSONB(), nullable=True
        ),  # actual response shape that broke
        sa.Column(
            "expected_sample", postgresql.JSONB(), nullable=True
        ),  # example output from a recent success
        sa.Column(
            "observed_sample", postgresql.JSONB(), nullable=True
        ),  # the failing output
        sa.Column(
            "upstream_inputs", postgresql.JSONB(), nullable=True
        ),  # inputs the node received
        # Telemetry
        sa.Column(
            "recent_success_count", sa.Integer(), nullable=False, server_default="0"
        ),
        sa.Column(
            "recent_failure_count", sa.Integer(), nullable=False, server_default="0"
        ),
        sa.Column(
            "created_at",
    ...
    op.create_index(
        "ix_prd_pipeline_created", "pipeline_run_diffs", ["pipeline_id", "created_at"]
    )
    op.create_index(
        "ix_prd_node_error",
        "pipeline_run_diffs",
    ...
    op.create_table(
        ...
        sa.Column(
            "confidence", sa.Numeric(3, 2), nullable=False, server_default="0.50"
        ),
        sa.Column(
    ...
    op.create_index(
        "ix_ppp_pipeline_status", "pipeline_patch_proposals", ["pipeline_id", "status"]
    )
    op.create_index(
        "ix_ppp_tenant_created", "pipeline_patch_proposals", ["tenant_id", "created_at"]
    )
