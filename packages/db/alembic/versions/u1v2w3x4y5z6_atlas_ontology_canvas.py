...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("owner_user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id"), nullable=False, index=True),
        sa.Column("kb_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("knowledge_collections.id"), nullable=True, index=True),
        sa.Column("name", sa.String(255), nullable=False, server_default="Untitled Atlas"),
    ...
    op.create_index("ix_atlas_graphs_tenant_updated", "atlas_graphs", ["tenant_id", "updated_at"])

    atlas_node_kind = postgresql.ENUM(
        "concept", "instance", "document", "property",
    ...
    op.create_table(
        ...
        sa.Column("graph_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("atlas_graphs.id", ondelete="CASCADE"), nullable=False, index=True),
        ...
        sa.Column("document_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("documents.id"), nullable=True, index=True),
    ...
    op.create_table(
        ...
        sa.Column("from_node_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("atlas_nodes.id", ondelete="CASCADE"), nullable=False),
        ...
        sa.Column("inverse_edge_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("atlas_edges.id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_index("ix_atlas_snapshots_graph_created", "atlas_snapshots", ["graph_id", "created_at"])
    op.create_unique_constraint("uq_atlas_snapshot_version", "atlas_snapshots", ["graph_id", "version"])
