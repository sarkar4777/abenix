...
async def ensure_table_and_seed():
    ...
    db_url = os.environ.get("DATABASE_URL", "postgresql://abenix:abenix@localhost:5432/abenix")
    ...
    """)
    await conn.execute("CREATE INDEX IF NOT EXISTS ix_subject_policies_lookup ON subject_policies(api_key_id, subject_type, subject_id);")
    await conn.execute("CREATE INDEX IF NOT EXISTS ix_subject_policies_api_key ON subject_policies(api_key_id);")
    ...
    default_rules = {
        "agents": {
            "mode": "allowlist",
            "slugs": ["example_app-chat", "example_app-pipeline", "example_app-deep-extractor"],
        },
        "knowledge_bases": [{
            "kb_id": "*",
        ...
        "tools": {
            "mode": "allowlist",
            "names": [
                "portfolio_example_app", "knowledge_search", "knowledge_store",
                "graph_explorer", "contract_search", "ppa_calculator",
                "entso_e", "ember_climate", "ecb_rates",
            ],
        },
        "data_scopes": {
    ...
    if existing:
        ...
        """, "the example app Default (All Users)",
            ...
            json.dumps(default_rules), existing['id'])
        print(f"  Updated default the example app policy")
    else:
        await conn.execute("""
        ...
        """, uuid.uuid4(), ciq_key['id'],
            "the example app Default (All Users)",
            "Default policy for all the example app users — isolates access to own data",
            json.dumps(default_rules))
        print(f"  Seeded default the example app policy")
