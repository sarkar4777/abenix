...
PRICING: dict[str, dict[str, float]] = {
    ...
    "claude-sonnet-4-6-20250106": {"input": 3.0 / 1_000_000, "output": 15.0 / 1_000_000},
...
_DEFAULT_OUTPUT_PER_M = float(_os.environ.get("LLM_DEFAULT_PRICING_OUTPUT_PER_M", "15.0"))
DEFAULT_PRICING = {
...
_DB_PRICING_TTL = 60.0  # seconds — long enough to amortise, short enough to pick up admin edits
    ...
    try:
        ...
        for model, inp, out, cached in rows:
            result[model] = {
                ...
                "cached_input": (float(cached) / 1_000_000) if cached is not None else None,
            }
        _DB_PRICING_CACHE = result
        _DB_PRICING_CACHE_AT = now
        return result
    except Exception as e:
        logger.debug("llm_model_pricing load failed, falling back to code constants: %s", e)
        return {}


def _calc_cost(model: str, input_tokens: int, output_tokens: int, latency_ms: int = 0) -> float:
...
def _emit_llm_metrics(
    ...
    try:
        ...
        _m.LLM_TOKENS.labels(provider=provider, model=model, direction="input").inc(input_tokens)
        _m.LLM_TOKENS.labels(provider=provider, model=model, direction="output").inc(output_tokens)
        _m.LLM_COST_USD.labels(provider=provider, model=model).inc(cost)
        _m.LLM_DURATION.labels(provider=provider, model=model).observe(duration_ms / 1000.0)
    except Exception:
        pass
...
class OpenAIProvider(LLMProvider):
    ...
    async def _stream(
        ...
        for buf in tool_calls_buf.values():
            try:
                args = json_mod.loads(buf["arguments_str"]) if buf["arguments_str"] else {}
    ...
    def _build_contents(
        ...
        for msg in messages:
            ...
            if isinstance(raw, list):
                ...
                if parts:
                    contents.append(
                        google_types.Content(role=role, parts=parts)
                    )
            else:
                contents.append(
        ...
        for attempt in range(3):
            try:
                ...
                if not stream and isinstance(result, LLMResponse):
                    llm_tokens_total.labels(model=model, direction="input").inc(result.input_tokens)
                    llm_tokens_total.labels(model=model, direction="output").inc(result.output_tokens)
                    llm_request_duration_seconds.labels(model=model, provider=provider_name).observe(result.latency_ms / 1000)
        ...
        if fallback_model and fallback_model != model:
            ...
            try:
                ...
                result = await fallback_provider.complete(
                    ...
                    model=fallback_model, temperature=temperature, stream=stream,
                    max_tokens=max_tokens,
                )
                if not stream and isinstance(result, LLMResponse):
                    _emit_llm_metrics(
                ...
                logger.error("llm_complete fallback to %s also failed: %s", fallback_model, fallback_error)
