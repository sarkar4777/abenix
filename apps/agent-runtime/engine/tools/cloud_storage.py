...
class CloudStorageTool(BaseTool):
    ...
    input_schema = {
        ...
        "properties": {
            "operation": {
                "type": "string",
                "enum": ["list_objects", "read_object", "write_object", "delete_object", "get_info"],
    ...
    async def execute(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        if not operation or not path:
            return ToolResult(content="Error: operation and path are required", is_error=True)
        ...
        try:
            ...
            if operation == "list_objects":
                ...
                return ToolResult(content=json.dumps({
                    "status": "success", "bucket": bucket, "prefix": prefix,
                ...
                try:
                    ...
                    if len(text) > 500_000:
                        text = text[:500_000] + "\n[Truncated at 500KB]"
                    return ToolResult(content=json.dumps({
                        "status": "success", "key": key, "size": len(body),
                        "content_type": resp.get("ContentType", ""), "content": text,
                    }))
                except UnicodeDecodeError:
                    import base64
                    return ToolResult(content=json.dumps({
                        "status": "success", "key": key, "size": len(body),
                        "content_type": resp.get("ContentType", ""), "encoding": "base64",
                ...
                return ToolResult(content=json.dumps({
                    "status": "success", "action": "written", "key": key, "size": len(content),
                }))

            elif operation == "delete_object":
                s3.delete_object(Bucket=bucket, Key=key)
                return ToolResult(content=json.dumps({
                    "status": "success", "action": "deleted", "key": key,
                }))
            ...
            return ToolResult(content="Error: boto3 not installed. Install with: pip install boto3", is_error=True)
    ...
    async def _gcs_operation(self, operation: str, path: str, args: dict) -> ToolResult:
        ...
        try:
            ...
            if operation == "list_objects":
                ...
                return ToolResult(content=json.dumps({
                    "status": "success", "bucket": bucket_name,
                ...
                return ToolResult(content=json.dumps({
                    "status": "success", "key": blob_name, "content": content,
                }))

            elif operation == "write_object":
                blob = bucket.blob(blob_name)
                blob.upload_from_string(args.get("content", ""))
                return ToolResult(content=json.dumps({"status": "success", "action": "written", "key": blob_name}))

            return ToolResult(content=f"Error: Unknown operation: {operation}", is_error=True)

        except ImportError:
            return ToolResult(content="Error: google-cloud-storage not installed", is_error=True)
        except Exception as e:
            return ToolResult(content=f"GCS error: {str(e)[:500]}", is_error=True)

    async def _azure_operation(self, operation: str, path: str, args: dict) -> ToolResult:
        """Azure Blob Storage operations."""
        try:
            from azure.storage.blob import BlobServiceClient
            conn_str = os.environ.get("AZURE_STORAGE_CONNECTION_STRING", "")
            if not conn_str:
                return ToolResult(content="Error: AZURE_STORAGE_CONNECTION_STRING not set", is_error=True)
            ...
            if operation == "list_objects":
                ...
                blobs = list(container_client.list_blobs(name_starts_with=args.get("prefix", "")))[:100]
                return ToolResult(content=json.dumps({
                    "status": "success", "container": container,
                    "objects": [{"key": b.name, "size": b.size} for b in blobs],
                }))
                ...
                if len(content) > 500_000:
                    content = content[:500_000] + "\n[Truncated]"
                return ToolResult(content=json.dumps({"status": "success", "content": content}))

            return ToolResult(content=f"Error: Unknown operation: {operation}", is_error=True)

        except ImportError:
            return ToolResult(content="Error: azure-storage-blob not installed", is_error=True)
        except Exception as e:
            return ToolResult(content=f"Azure error: {str(e)[:500]}", is_error=True)

    async def _local_operation(self, operation: str, path: str, args: dict) -> ToolResult:
        ...
        if operation == "list_objects":
            if not local_path.is_dir():
                return ToolResult(content=f"Error: {local_path} is not a directory", is_error=True)
            files = sorted(local_path.iterdir())[:100]
            return ToolResult(content=json.dumps({
                "status": "success", "path": str(local_path),
                "objects": [{"key": f.name, "size": f.stat().st_size if f.is_file() else 0, "type": "file" if f.is_file() else "dir"} for f in files],
            }))

        elif operation == "read_object":
            if not local_path.is_file():
                return ToolResult(content=f"Error: {local_path} not found", is_error=True)
            ...
            return ToolResult(content=json.dumps({"status": "success", "action": "written", "path": str(local_path)}))
