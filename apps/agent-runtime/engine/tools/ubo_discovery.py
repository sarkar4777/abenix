...
class UBODiscoveryTool(BaseTool):
    ...
    input_schema: dict[str, Any] = {
        ...
        "properties": {
            ...
            "sources": {
                ...
                "items": {
                    "type": "string",
                    "enum": ["gleif", "opencorporates", "uk_psc", "pl_krs"],
                },
    ...
    async def execute(self, arguments: dict[str, Any]) -> ToolResult:
            ...
            if candidates:
                ...
                if parents.get("direct_parent"):
                    ...
                    discovery_gaps.append(
                        {
                            "node": dp["lei"],
                            "reason": "GLEIF provides parent identity but not ownership % — request official register filing for exact stake.",
                        }
                    )
                if parents.get("ultimate_parent") and (
                    not parents.get("direct_parent")
                    or parents["ultimate_parent"]["lei"]
                    != parents["direct_parent"]["lei"]
                ):
        ...
        if "opencorporates" in sources:
            ...
            if oc_candidates:
                ...
                if best.get("company_number") and best.get("jurisdiction"):
                    oc_officers = await _opencorporates_officers(
                        best["jurisdiction"], best["company_number"]
                    )
            else:
                    ...
                    ownership_tree["nodes"].append(
                        {
                            ...
                            "type": (
                                "individual"
                                if (owner.get("kind") or "").startswith("individual")
                                else "entity"
                            ),
        ...
        if "pl_krs" in sources and country == "PL":
            ...
            if not pl_krs_data:
                warnings.append(
                    "Polish KRS: lookup returned no structured data — try the web KRS viewer manually."
                )
        ...
        if not ubos and oc_officers:
            for off in oc_officers:
                ...
                if any(
                    k in position
                    for k in ["director", "owner", "shareholder", "partner", "manager"]
                ):
            ...
            content=json.dumps(
                {
                    ...
                    "legal_entity_resolved": gleif_record
                    or (oc_candidates[0] if oc_candidates else None),
                    "gleif_parents": parents,
                    "opencorporates_candidates": oc_candidates[:3],
                    "uk_psc_beneficial_owners": uk_psc_owners,
                    "opencorporates_active_officers": [
                        o for o in oc_officers if o.get("is_active")
                    ][:15],
