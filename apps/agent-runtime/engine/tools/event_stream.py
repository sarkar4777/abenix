...
class EventBufferTool(BaseTool):
    ...
    input_schema = {
        "type": "object",
        "properties": {
            "source": {"type": "string", "description": "Event source identifier (trigger name or 'all')"},
            "event_type": {"type": "string", "description": "Filter by event type (optional)"},
            "limit": {"type": "integer", "default": 100, "description": "Max events to read"},
            "since_seconds": {"type": "integer", "default": 3600, "description": "Only events from last N seconds"},
            "consume": {"type": "boolean", "default": True, "description": "Mark events as consumed after reading"},
        },
        "required": [],
    }
    ...
    async def execute(self, arguments: dict[str, Any]) -> Any:
        ...
        try:
            ...
            raw_events = await r.zrangebyscore(key, min=cutoff, max="+inf", start=0, num=limit)
            ...
            await r.aclose()
            return {"events": events, "count": len(events), "source": source, "consumed": consume}
...
class RedisStreamConsumerTool(BaseTool):
    ...
    input_schema = {
        "type": "object",
        "properties": {
            "stream": {"type": "string", "description": "Redis stream name (e.g., 'sensor:temperature', 'orders:new')"},
            "group": {"type": "string", "description": "Consumer group name (created if not exists)"},
            "consumer": {"type": "string", "description": "Consumer name within the group"},
            "count": {"type": "integer", "default": 10, "description": "Max messages to read"},
            "block_ms": {"type": "integer", "default": 0, "description": "Block for N ms waiting for messages (0 = no block)"},
            "acknowledge": {"type": "boolean", "default": True, "description": "Acknowledge messages after reading"},
        },
        "required": ["stream"],
    }
        ...
        try:
            ...
            try:
                ...
                if "BUSYGROUP" not in str(e):
                    return {"error": f"Failed to create consumer group: {e}", "messages": [], "count": 0}
            ...
            await r.aclose()
            return {"messages": messages, "count": len(messages), "stream": stream, "group": group}
        except Exception as e:
...
class RedisStreamPublisherTool(BaseTool):
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "maxlen": {"type": "integer", "default": 10000, "description": "Max stream length (oldest trimmed)"},
        },
    ...
    async def execute(self, arguments: dict[str, Any]) -> Any:
        ...
        flat = {str(k): json.dumps(v) if not isinstance(v, str) else v for k, v in data.items()}
...
class KafkaConsumerTool(BaseTool):
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "group_id": {"type": "string", "default": "abenix", "description": "Consumer group ID"},
            "max_messages": {"type": "integer", "default": 10, "description": "Max messages to consume"},
            "timeout_ms": {"type": "integer", "default": 5000, "description": "Poll timeout in milliseconds"},
            "from_beginning": {"type": "boolean", "default": False, "description": "Start from beginning of topic"},
        },
        "required": ["topic"],
    }
    ...
    async def execute(self, arguments: dict[str, Any]) -> Any:
        ...
        if not bootstrap:
            return {"error": "KAFKA_BOOTSTRAP_SERVERS not configured", "messages": [], "count": 0}
        ...
        try:
            ...
            return {"error": "aiokafka not installed. Run: pip install aiokafka", "messages": [], "count": 0}
        except Exception as e:
            return {"error": str(e), "messages": [], "count": 0}
