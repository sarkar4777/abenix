...
async def _aws_summary() -> dict[str, Any]:
    if not os.environ.get("AWS_ACCESS_KEY_ID"):
        return {"provider": "aws", "skipped": True,
    ...
    try:
        ...
        return {"provider": "aws", "period": {"start": start, "end": end},
                "total_usd": round(total, 2), "by_service": rows}
...
async def _gcp_summary() -> dict[str, Any]:
    ...
    if not (project and dataset):
        return {"provider": "gcp", "skipped": True,
                "reason": "GCP_BILLING_PROJECT and GCP_BILLING_BQ_DATASET not set"}
    try:
        from google.cloud import bigquery  # type: ignore
    except ImportError:
        return {"provider": "gcp", "skipped": True, "reason": "google-cloud-bigquery not installed"}
    try:
        ...
        sql = f"""
            ...
            LIMIT 100
        """
        rows = [{"service": r["service"], "cost_usd": round(float(r["cost_usd"] or 0), 2)} for r in client.query(sql).result()]
...
async def _azure_summary() -> dict[str, Any]:
    ...
    if not sub:
        return {"provider": "azure", "skipped": True,
    ...
    try:
        ...
        agg: dict[str, float] = {}
        for u in usage:
            svc = (getattr(u, "instance_name", None) or getattr(u, "consumed_service", None) or "unknown")
            cost = float(getattr(u, "cost_in_usd", 0) or 0)
            agg[svc] = agg.get(svc, 0.0) + cost
        rows = [{"service": k, "cost_usd": round(v, 2)} for k, v in sorted(agg.items(), key=lambda kv: -kv[1])]
        return {"provider": "azure", "total_usd": round(sum(r["cost_usd"] for r in rows), 2), "by_service": rows}
    except Exception as e:
    ...
    async def execute(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        try:
            if op == "aws_summary":
                ...
                results = [await _aws_summary(), await _gcp_summary(), await _azure_summary()]
            else:
        ...
        return ToolResult(content="\n".join(lines), metadata={"providers": results,
                                                                  "combined_total_usd": round(total, 2)})
