...
def _classify_text(title: str, snippet: str) -> tuple[list[str], str]:
    ...
    if any(k in blob for k in _ADVERSE_KEYWORDS):
        ...
        k in blob
        for k in ["acquit", "cleared", "dropped charges", "no wrongdoing", "exonerat"]
    ):
        stance = "positive"
    return cats, stance
...
async def _gdelt_adverse(name: str) -> tuple[list[dict[str, Any]], str | None]:
    ...
    adverse_terms = (
        "(lawsuit OR fraud OR investigation OR penalty OR scandal OR corruption)"
    )
    quoted_name = '"' + name + '"'
...
async def _google_news_rss(name: str) -> tuple[list[dict[str, Any]], str | None]:
    ...
    for item in channel.findall("item")[:25]:
        ...
        try:
            ...
            iso = (
                parsedate_to_datetime(pub).astimezone(timezone.utc).isoformat()
                if pub
                else None
            )
        except Exception:
...
class AdverseMediaTool(BaseTool):
    ...
    async def execute(self, arguments: dict[str, Any]) -> ToolResult:
                    ...
                    content=json.dumps(
                        {
                            "queried_name": name,
                            "from_cache": True,
                            **self._summarize(name, hits, max_results),
                        },
                        indent=2,
                    ),
            ...
            enriched.append(
                {
                    **h,
                    "name_match_confidence": max(
                        score, _fuzzy(name, h.get("snippet", ""))
                    ),
        ...
        for h in sorted(
            enriched,
            key=lambda x: (x["source_weight"], x["name_match_confidence"]),
            reverse=True,
        ):
    ...
    @staticmethod
    def _summarize(
        name: str, hits: list[dict[str, Any]], max_results: int
    ) -> dict[str, Any]:
        ...
        recent_tier3_plus = [
            h
            for h in adverse
            if h["source_weight"] >= 3
            and h["recency_bucket"] in ("30d", "90d", "1y", "3y")
        ]
