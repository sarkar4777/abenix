...
class TimeSeriesAnalyzerTool(BaseTool):
    ...
    input_schema: dict[str, Any] = {
        ...
        "properties": {
            "data": {
                "type": "array",
                "items": {"type": "number"},
                "description": "Array of numeric values (time-ordered)",
            },
            "timestamps": {
                "type": "array",
                "items": {"type": "string"},
            ...
            "compare_data": {
                "type": "array",
                "items": {"type": "number"},
                "description": "Second series for correlation analysis",
            },
        },
    ...
    async def execute(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        if len(data) < 2 and operation != "statistics":
            return ToolResult(
                content=f"Operation '{operation}' needs at least 2 data points, got {len(data)}",
                is_error=True,
            )
        ...
        try:
            if operation == "statistics":
                result = {
                    ...
                    "change_pct": (
                        float((arr[-1] - arr[0]) / arr[0] * 100) if arr[0] != 0 else 0
                    ),
                }
                ...
                result = {
                    "moving_average": [round(float(v), 4) for v in ma],
                    "window": window,
                }
                ...
                if std == 0:
                    ...
                    anomalies = [
                        {"index": i, "value": float(arr[i]), "z_score": round(z, 2)}
                ...
                if not compare or len(compare) != len(data):
                    return ToolResult(
                        content="compare_data must have same length as data",
                        is_error=True,
                    )
