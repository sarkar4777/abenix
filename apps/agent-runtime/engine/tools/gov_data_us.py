...
class GovDataUSTool(BaseTool):
    ...
    input_schema: dict[str, Any] = {
        ...
        "properties": {
            "operation": {
                "type": "string",
                "enum": ["lookup_company", "get_filing_text"],
    ...
    async def execute(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                content=f"SEC HTTP {e.response.status_code}: {e.response.text[:200]}",
                is_error=True,
            )
    ...
    async def _lookup(
        ...
        if not query:
            return ToolResult(
                content="query is required for lookup_company", is_error=True
            )
        ...
        for entry in tickers.values():
            if (
                entry.get("ticker", "").lower() == q_lower
                or q_lower in entry.get("title", "").lower()
            ):
                match = entry
                break
        if not match:
        ...
        lines = [
            ...
            f"Recent filings: {len(rows)}"
            + (f" (filtered to {form_type})" if form_type else ""),
            "",
        ]
        for r in rows:
            lines.append(
                f"  {r['filing_date']}  {r['form']:<10}  {r['accession_number']}"
            )
        return ToolResult(
            content="\n".join(lines),
    ...
    async def _get_filing(
        ...
        try:
            ...
            return ToolResult(
                content="query must be a numeric CIK for get_filing_text", is_error=True
            )
