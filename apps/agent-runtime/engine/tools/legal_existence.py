...
RED_FLAGS = {
    "shell_pattern_recent_incorporation":
        "Incorporated <6 months ago yet being used for a material transaction.",
    "shell_pattern_mass_address":
        "Registered address matches a well-known mass-registration service.",
    "shell_pattern_dormant":
        "No trading history / nil accounts for two or more consecutive years.",
    "jurisdiction_mismatch":
        "Incorporation country differs from stated operating country with no branch filing.",
    "dissolved_or_struck_off":
        "Register indicates the entity is dissolved, liquidated, or struck off.",
    "no_registry_match":
        "No match in any of the public registries — manual verification mandatory.",
    "multiple_registry_matches_same_jurisdiction":
        "Multiple entities share the exact same name in the same jurisdiction — disambiguate before proceeding.",
    "lei_lapsed":
        "LEI exists but status is LAPSED — LEI renewal overdue.",
    "unusual_legal_form":
        "Legal form is one typically associated with tax-shelter structures (trust companies, IBCs, cell companies).",
}
...
async def _gleif_lookup(name: str, country: str | None) -> list[dict[str, Any]]:
    ...
    for item in data.get("data", []) or []:
        ...
        out.append({
            ...
            "status": (attrs.get("registration") or {}).get("status") or entity.get("status"),
...
async def _opencorporates_lookup(name: str, country: str | None) -> list[dict[str, Any]]:
...
async def _uk_ch_lookup(name: str) -> list[dict[str, Any]]:
    ...
    for item in data.get("items", []) or []:
        out.append({
            ...
            "registered_address": (item.get("address") or {}).get("premises", "") + " " + (item.get("address") or {}).get("address_line_1", "") + ", " + (item.get("address") or {}).get("locality", "") + " " + (item.get("address") or {}).get("postal_code", ""),
...
class LegalExistenceVerifierTool(BaseTool):
    ...
    async def execute(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        if gleif_hits:
            trail.append({
                "source": "GLEIF",
                "url": f"https://www.gleif.org/en/lei-data/gleif-lei-look-up-api/",
        ...
        source_of_best = ""
        if uk_hits:
            best = uk_hits[0]; source_of_best = "UK Companies House"
        elif oc_hits:
            best = oc_hits[0]; source_of_best = "OpenCorporates"
        elif gleif_hits:
            best = gleif_hits[0]; source_of_best = "GLEIF"
        ...
        status = "unknown"; incorporation_date = None; address = None; reg_num = None; lei = None
        legal_form = None
        if best:
            status = _classify_status(best.get("status"))
            incorporation_date = best.get("incorporation_date")
            address = best.get("registered_address") or " ".join(best.get("address_lines") or [])
            reg_num = best.get("company_number") or best.get("registration_number") or reg_number
            ...
            if legal_form and any(k in legal_form.lower() for k in ["ibc", "international business", "trust compan", "cell compan", "segregated portfolio"]):
        ...
        return ToolResult(
            content=json.dumps({
                ...
                "red_flag_descriptions": {f: RED_FLAGS.get(f, "") for f in sorted(set(red_flags))},
