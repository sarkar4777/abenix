...
class DynamicTool(BaseTool):
    ...
    def __init__(self, tool_name: str, tool_description: str,
                 tool_code: str, tool_params: list[dict[str, Any]],
    ...
    async def execute(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        if errors:
            return ToolResult(content=f"Code validation failed: {'; '.join(errors)}", is_error=True)
...
async def generate_dynamic_tool(
    ...
    if not tool_name:
        tool_name = "custom_" + description[:30].lower().replace(" ", "_").replace("-", "_")
...
```"""
    ...
    try:
        ...
        if review_result.get("issues"):
            improved_code = await _improve_code(llm, code, review_result["issues"], description)
            if improved_code:
...
async def _adversarial_review(llm: Any, code: str, description: str, params: list) -> dict:
...
async def _improve_code(llm: Any, code: str, issues: list[str], description: str) -> str | None:
