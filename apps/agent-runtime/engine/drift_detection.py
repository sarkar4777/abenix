...
class DriftDetector:
    ...
    async def capture_baseline(self, agent_id: str) -> BaselineMetrics:
        ...
        baseline = BaselineMetrics(
            ...
            std_duration_ms=std_d,
            avg_input_tokens=avg_it,
            std_input_tokens=std_it,
            avg_output_tokens=avg_ot,
            std_output_tokens=std_ot,
            avg_cost=avg_c,
            std_cost=std_c,
            avg_confidence=avg_conf,
            std_confidence=std_conf,
            avg_output_length=avg_ol,
            std_output_length=std_ol,
            tool_failure_rate=avg_tfr,
            std_tool_failure_rate=std_tfr,
            sample_count=n,
        )
    ...
    async def get_baseline(self, agent_id: str) -> BaselineMetrics:
        ...
        data = json.loads(raw)
        return BaselineMetrics(
            **{k: data[k] for k in BaselineMetrics.__dataclass_fields__ if k in data}
        )
    ...
    def _check_drift(
        ...
        **current_metrics: float,
        ...
        metric_map = [
            ...
            (
                "tool_failure_rate",
                baseline.tool_failure_rate,
                baseline.std_tool_failure_rate,
            ),
        ]
