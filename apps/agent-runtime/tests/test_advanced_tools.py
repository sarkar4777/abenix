...
class TestDocumentExtractorTool:
    ...
    async def test_extract_key_values_money(self, extractor):
        text = (
            "The base price is $45.50/MWh with a total contract value of $12,500,000."
        )
        result = await extractor.execute({"text": text, "extract_type": "key_values"})
        data = json.loads(result.content)
    ...
    async def test_extract_from_file(self, extractor):
        with tempfile.NamedTemporaryFile(suffix=".txt", mode="w", delete=False) as f:
            ...
            f.flush()
            result = await extractor.execute(
                {"file_path": f.name, "extract_type": "all"}
            )
        os.unlink(f.name)
        assert not result.is_error
...
class TestCsvAnalyzerTool:
    ...
    async def test_describe(self, analyzer, sample_csv):
        result = await analyzer.execute(
            {"csv_text": sample_csv, "operation": "describe"}
        )
        assert not result.is_error
        data = json.loads(result.content)
    ...
    async def test_quality(self, analyzer, sample_csv):
        result = await analyzer.execute(
            {"csv_text": sample_csv, "operation": "quality"}
        )
        data = json.loads(result.content)
    ...
    async def test_file_read(self, analyzer, sample_csv):
        with tempfile.NamedTemporaryFile(suffix=".csv", mode="w", delete=False) as f:
            ...
            f.flush()
            result = await analyzer.execute(
                {"file_path": f.name, "operation": "describe"}
            )
        os.unlink(f.name)
        assert not result.is_error
...
class TestUnitConverterTool:
    ...
    async def test_energy_mwh_to_kwh(self, converter):
        result = await converter.execute(
            {"value": 1, "from_unit": "MWh", "to_unit": "kWh"}
        )
        data = json.loads(result.content)
    ...
    async def test_energy_gwh_to_mwh(self, converter):
        result = await converter.execute(
            {"value": 1, "from_unit": "GWh", "to_unit": "MWh"}
        )
        data = json.loads(result.content)
        assert data["result"] == 1000
    ...
    async def test_power_mw_to_kw(self, converter):
        result = await converter.execute(
            {"value": 1, "from_unit": "MW", "to_unit": "kW"}
        )
        data = json.loads(result.content)
        assert data["result"] == 1000
    ...
    async def test_unknown_units(self, converter):
        result = await converter.execute(
            {"value": 1, "from_unit": "xyz", "to_unit": "abc"}
        )
        assert result.is_error
...
class TestMarketDataTool:
    ...
    async def test_economic_indicator_mock(self, market):
        result = await market.execute(
            {"data_type": "economic_indicator", "symbol": "GDP"}
        )
        data = json.loads(result.content)
...
class TestSpreadsheetAnalyzerTool:
    ...
    async def test_csv_overview(self, analyzer):
        with tempfile.NamedTemporaryFile(suffix=".csv", mode="w", delete=False) as f:
            ...
            f.flush()
            result = await analyzer.execute(
                {"file_path": f.name, "operation": "overview"}
            )
        os.unlink(f.name)
        data = json.loads(result.content)
    ...
    async def test_csv_statistics(self, analyzer):
        with tempfile.NamedTemporaryFile(suffix=".csv", mode="w", delete=False) as f:
            ...
            f.flush()
            result = await analyzer.execute(
                {"file_path": f.name, "operation": "statistics"}
            )
        os.unlink(f.name)
        data = json.loads(result.content)
    ...
    async def test_csv_search(self, analyzer):
        with tempfile.NamedTemporaryFile(suffix=".csv", mode="w", delete=False) as f:
            ...
            f.flush()
            result = await analyzer.execute(
                {"file_path": f.name, "operation": "search", "search_term": "active"}
            )
        os.unlink(f.name)
        data = json.loads(result.content)
...
class TestNodeTrace:
    ...
    def test_execution_result_traces(self):
        ...
        traces = [
            NodeTrace(
                node_id="n0", node_type="user_input", iteration=0, timestamp_ms=1000
            ),
            NodeTrace(
                node_id="n1", node_type="llm_call", iteration=0, timestamp_ms=1100
            ),
            NodeTrace(
                node_id="n2", node_type="tool_call", iteration=0, timestamp_ms=1200
            ),
        ]
