...
async def test_create_mcp_connection(client: AsyncClient):
    ...
    resp = await client.post("/api/mcp/connections", json={
...
async def test_list_mcp_connections(client: AsyncClient):
    token = await _register(client)
    await client.post("/api/mcp/connections", json={
        "server_name": "MCP One",
...
async def test_get_mcp_connection(client: AsyncClient):
    token = await _register(client)
    create_resp = await client.post("/api/mcp/connections", json={
        "server_name": "Get MCP",
...
async def test_update_mcp_connection(client: AsyncClient):
    ...
    resp = await client.put(f"/api/mcp/connections/{conn_id}", json={
