...
from app.services.workflow_shell_grammar import verb_doc_md
...
async def _verb_show(
    *, args: dict[str, Any], db: AsyncSession, agent: Any, user: Any
) -> dict[str, Any]:
    ...
    if obj == "nodes":
        ...
        rows = [
            {
                "id": n.get("id"),
                "tool": n.get("tool") or n.get("agent_slug"),
...
async def _verb_describe(
    *, args: dict[str, Any], agent: Any, **_: Any
) -> dict[str, Any]:
    nid = args.get("node")
    nodes = (agent.model_config_ or {}).get("pipeline_config", {}).get("nodes", [])
    for n in nodes:
...
async def _verb_diff(
    *, args: dict[str, Any], db: AsyncSession, agent: Any, user: Any
    ...
    return {
        "kind": "json",
        "title": f"diff {args.get('a')} vs {args.get('b')}",
        "body": {
            "a": {
                "id": str(a.id),
                "status": str(a.status),
                ...
                "output_preview": (a.output_message or "")[:600],
            },
            "b": {
                "id": str(b.id),
                "status": str(b.status),
                ...
                "output_preview": (b.output_message or "")[:600],
            },
            "delta": {
...
async def _verb_list(
    *, args: dict[str, Any], db: AsyncSession, agent: Any, user: Any
) -> dict[str, Any]:
    cat = (args.get("category") or "").lower()
    if cat == "patches":
        return await _verb_show(
            args={"object": "patches"}, db=db, agent=agent, user=user
        )
    if cat == "runs":
        ...
        rows = (
            (
                await db.execute(
                    ...
                    .order_by(desc(AgentRevision.created_at))
                    .limit(20)
                )
            )
            .scalars()
...
def _patch_set(path: str, value: Any) -> list[dict[str, Any]]:
    ...
    return [
        {
            "op": "replace",
            "path": f"/pipeline_config/nodes/by-id={node}/{field}",
            "value": value,
        }
    ]
...
def _apply_patches_locally(
    dsl: dict[str, Any], ops: list[dict[str, Any]]
) -> dict[str, Any]:
...
def build_mutation_patch(
    verb: str, args: dict[str, Any], dsl: dict[str, Any]
) -> dict[str, Any]:
    ...
    if verb == "set":
        ...
        ops = [
            {"op": "replace", "path": f"/pipeline_config/nodes/{idx}/id", "value": new}
        ]
        for j, n in enumerate(nodes):
            if old in (n.get("depends_on") or []):
                ...
                ops.append(
                    {
                        "op": "replace",
                        "path": f"/pipeline_config/nodes/{j}/depends_on",
                        "value": deps,
                    }
                )
        ...
        ops = [
            {
                "op": "replace",
                "path": f"/pipeline_config/nodes/{idx}/model",
                "value": model,
            }
        ]
        ...
        ops = [
            {
                "op": "replace",
                "path": f"/pipeline_config/nodes/{idx}/input_mappings",
                "value": mappings,
            }
        ]
        ...
        if dependants:
            raise ValueError(
                f"cannot remove '{nid}' — depended on by: {', '.join(dependants)}"
            )
        ...
        ops = [{"op": "add", "path": "/pipeline_config/nodes/-", "value": new_node}]
