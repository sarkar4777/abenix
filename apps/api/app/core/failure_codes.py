...
_RULES: list[tuple[str, str]] = [
    ...
    # timeout rule below.
    (r"stuck\s+in\s+running|sweep.*backfill|owning\s+process\s+likely\s+crashed", "STALE_SWEEP"),

    # LLM provider errors
    (r"rate.?limit|429|too\s*many", "LLM_RATE_LIMIT"),
    (r"anthropic.*(error|exception)|openai.*(error|exception)|gemini.*(error|exception)", "LLM_PROVIDER_ERROR"),
    ...
    (r"moderation\s*blocked|policy\s*triggered|content\s*violation", "MODERATION_BLOCKED"),
...
def emit_outcome_metric(
    *, outcome: str, failure_code: str = "", agent_type: str = "agent",
) -> None:
