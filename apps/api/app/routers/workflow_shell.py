...
from app.services.workflow_shell_grammar import (
    GRAMMAR,
    list_verbs,
    parse_command,
    verb_doc_md,
...
async def _nl_translate(text: str) -> dict[str, Any]:
    ...
    model = await get_setting(
        "workflow_shell.model", default="claude-sonnet-4-5-20250929"
    )
    ...
    if verb in EXECUTE_VERBS:
        ...
        if verb == "merge":
            return success(
                {
                    "kind": "execution",
                    "body": f"Merge '{args.get('branch')}' (coming soon).",
                }
            )
        if verb == "rollback":
            return success(
                {
                    "kind": "execution",
                    "body": f"Rollback target '{args.get('target')}' — use the Healing tab Roll Back button for now.",
                }
            )
        ...
        return success(
            {
                ...
                "body": {
                    "verb": verb,
                    "args": args,
                    "note": "GOVERN verb registered; runtime persistence coming in Phase 2.5",
                },
            }
        )
