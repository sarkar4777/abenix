...
def _serialize(row: LLMModelPricing) -> dict[str, Any]:
    return {
        ...
        "cached_input_per_m": float(row.cached_input_per_m) if row.cached_input_per_m is not None else None,
        "batch_input_per_m": float(row.batch_input_per_m) if row.batch_input_per_m is not None else None,
        "batch_output_per_m": float(row.batch_output_per_m) if row.batch_output_per_m is not None else None,
...
async def create_pricing(
    ...
    if not model or provider not in {"anthropic", "openai", "google", "other"}:
        return error("model and provider (anthropic/openai/google/other) are required", 400)

    try:
        input_per_m = float(body["input_per_m"])
        output_per_m = float(body["output_per_m"])
    except (KeyError, TypeError, ValueError):
        return error("input_per_m and output_per_m must be numbers ($ per 1M tokens)", 400)
...
async def update_pricing(
    ...
    for field in (
        ...
        if field in body:
            try:
                setattr(row, field, float(body[field]) if body[field] is not None else None)
...
async def seed_from_defaults(
    ...
    existing = {
        r[0] for r in (await db.execute(text("SELECT model FROM llm_model_pricing"))).all()
    }
    added = 0
    for model, provider, inp, out, cached, b_in, b_out in baseline:
        ...
        db.add(LLMModelPricing(
            ...
            batch_input_per_m=b_in, batch_output_per_m=b_out,
        ))
        added += 1
    await db.commit()
